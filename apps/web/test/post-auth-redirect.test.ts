...
describe('post auth redirect helpers', () => {
  it('detects an embedded Hugging Face Space host', () => {
    expect(
      isEmbeddedHuggingFaceSpace('christopherjkoen-zeeus-ultimate-site.hf.space', true)
    ).toBe(true);
  });

  it('does not treat a top-level Space as embedded auth', () => {
    expect(
      isEmbeddedHuggingFaceSpace('christopherjkoen-zeeus-ultimate-site.hf.space', false)
    ).toBe(false);
  });
