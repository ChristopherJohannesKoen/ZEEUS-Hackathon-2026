  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col animate-slide-up">
        {/* Header */}
        <div
          className="p-5 pb-4 flex items-start justify-between"
          style={{ backgroundColor: def.color + '20', borderBottom: `3px solid ${def.color}` }}
        >
...
export default function SDGAlignmentPage({ params }: Props) {
  ...
  const [selectedSDG, setSelectedSDG] = useState<{ number: number; source: SDGSource } | null>(
    null
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {alignment.length} SDGs identified for {evaluation.context.name} — combining stage and
      ...
      <Card className="mb-6 bg-brand/5 border-l-4 border-l-brand">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-gray-600 leading-relaxed">
            <strong className="text-brand-dark">Stage SDGs</strong> are derived from your current
            startup stage ({evaluation.context.stage}).{' '}
            <strong className="text-brand-dark">Business SDGs</strong> come from your NACE division
            ({evaluation.context.naceCode}). <strong className="text-brand-dark">Both</strong> means
            the SDG is reinforced by both signals — higher priority.
            <br />
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 text-gray-400">No SDGs match the selected filter.</div>
      )}
