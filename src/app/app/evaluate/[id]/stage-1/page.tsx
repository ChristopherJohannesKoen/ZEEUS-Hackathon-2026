...
import { calcImpactScore, scoreToBand, bandLabel, bandColor, calcFinancialTotal } from "@/lib/scoring";
import type { ESGTopicAssessment, FinancialLevel, DimensionScore, LikelihoodValue, EvidenceBasis } from "@/types/evaluation";
import { Card } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { PriorityChip } from "@/components/ui/Badge";
import { ScoreBar } from "@/components/ui/ProgressBar";
import {
  ArrowRight, ChevronDown, ChevronUp, Info, HelpCircle,
  DollarSign, Leaf, Users, CheckCircle
} from "lucide-react";
      ...
      {open && (
        <div className="px-4 pb-4 text-xs text-gray-600 bg-brand/3 space-y-2">
          ...
          <ul className="space-y-1 list-disc pl-4">
            <li><strong>Magnitude</strong> — severity of the impact if it occurs (1–4)</li>
            <li><strong>Scale</strong> — how many people/areas are affected (1–4)</li>
            <li><strong>Irreversibility</strong> — how hard is it to fix (1–4)</li>
            <li><strong>Likelihood</strong> — probability of it occurring (25–100%)</li>
          </ul>
          <p className="text-gray-500">
            Score ≥ 2 = material topic. Score ≥ 2.5 = high priority. Mark N/A if the topic genuinely doesn't apply.
          </p>
        </div>
      )}
    </div>
...
function ESGTopicRow({
  ...
  return (
    <div className={cn(
      ...
      assessment.applicable ? "border-surface-border bg-white" : "border-dashed border-gray-200 bg-gray-50/50",
      ...
      <div className="flex items-center gap-3 p-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="font-bold text-sm text-gray-900">{topic.code} — {topic.title}</span>
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer text-xs">
            ...
            <span className={assessment.applicable ? "text-brand-dark font-semibold" : "text-gray-400"}>
      ...
      {expanded && assessment.applicable && (
        <div className="px-4 pb-4 border-t border-surface-border pt-4 space-y-4 animate-fade-in">
          ...
          <div className="rounded-xl bg-brand/5 p-3">
            <p className="text-[11px] font-bold text-brand-dark mb-2">Indicator-specific guide for {topic.title}:</p>
            <div className="grid grid-cols-2 gap-1.5">
              {[1,2,3,4].map((level) => (
                <div key={level} className="text-[11px] text-gray-600">
                  <span className="font-semibold">{["Low","Moderate","Significant","High"][level-1]}: </span>
          ...
          <div>
            <label className="text-xs font-semibold text-gray-700 block mb-1.5">
              Evidence basis
              <span className="text-gray-400 font-normal ml-1">— how confident are you in these inputs?</span>
            </label>
          ...
          <div className={cn(
            ...
            band === "high" ? "bg-red-50 border border-red-100" :
            band === "relevant" ? "bg-amber-50 border border-amber-100" :
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">
                {band === "high" ? "Strategic action recommended." :
                 band === "relevant" ? "Review recommendations for this topic." :
                 band === "low" ? "Monitor and reassess later." :
                 band === "verylow" ? "Minimal impact — no action needed now." :
...
export default function Stage1Page({ params }: Props) {
  ...
  const { getEvaluation, updateStage1, updateESGTopic, initializeStage1IfNeeded } = useEvaluationStore();
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Inside-out: Financial, Environmental, and Social/Governance indicators for {evaluation.context.name}.
          </p>
        </div>
      ...
      {activeSection === "financial" && (
        <div className="space-y-4 animate-fade-in">
          ...
          {FINANCIAL_KPIS.map((kpi) => {
            const currentLevel = stage1.financial[kpi.id as keyof typeof stage1.financial] as number;
            return (
              <Card key={kpi.id}>
                <div className="mb-3">
                  ...
                  {([0,1,2,3] as FinancialLevel[]).map((level) => (
                    ...
                    >
                      <div className={cn(
                        ...
                        {currentLevel === level && <div className="h-2 w-2 rounded-full bg-white" />}
                      </div>
                      <div>
                        <span className={cn(
                          "text-xs font-bold",
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          {envTopics.map((topic) => (
            <ESGTopicRow
              key={topic.id}
              topic={topic}
              assessment={stage1.environmental[topic.id] ?? {
                topicId: topic.id, applicable: true, magnitude: 1, scale: 1, irreversibility: 1,
                likelihood: 0.5, evidenceBasis: "assumed"
              }}
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          {socTopics.map((topic) => (
            <ESGTopicRow
              key={topic.id}
              topic={topic}
              assessment={stage1.social[topic.id] ?? {
                topicId: topic.id, applicable: true, magnitude: 1, scale: 1, irreversibility: 1,
                likelihood: 0.5, evidenceBasis: "assumed"
              }}
              onChange={(data) => updateESGTopic(id, topic.id, data)}
