...
import {
  RISK_DEFINITIONS, OPPORTUNITY_DEFINITIONS,
  PROB_LABELS, IMPACT_LABELS,
} from "@/data/risks-opportunities";
import { calcRiskRating, calcOpportunityRating, calcOverallRisk, calcOverallOpportunity } from "@/lib/scoring";
  ...
  return (
    <div className="flex gap-1">
      {Array.from({ length: size }).map((_, i) => (
        <div key={i} className={cn("h-2 w-2 rounded-full", i < value ? colors[value] : "bg-gray-100")} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-black text-gray-900 mb-1">Stage II — Risks & Opportunities</h1>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <Card className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-lg font-black text-gray-900">{overallRisk.score.toFixed(1)}</span>
        ...
        <Card className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-lg font-black text-gray-900">{overallOpp.score.toFixed(1)}</span>
      ...
      <div className="flex gap-2 mb-6">
        {[
          { key: "risks" as const, label: "Risks", icon: <Shield size={14} />, count: topRisks.length },
          { key: "opportunities" as const, label: "Opportunities", icon: <TrendingUp size={14} />, count: topOpps.length },
        ].map((tab) => (
          <button
            key={tab.key}
          ...
          >
            ...
            {tab.count > 0 && (
              <span className={cn(
                ...
                activeTab === tab.key ? "bg-white/20 text-white" : "bg-surface-muted text-gray-600"
              )}>
                {tab.count} active
              </span>
            )}
      ...
      {activeTab === "risks" && (
        <div className="space-y-3 animate-fade-in">
          <div className="rounded-xl bg-red-50 border border-red-100 p-3 text-xs text-red-700 flex items-start gap-2 mb-4">
            ...
            For each risk, select Probability (how likely?) and Impact (how severe?). The matrix produces the rating automatically.
          </div>

          {RISK_DEFINITIONS.map((riskDef) => {
            const riskData = stage2.risks[riskDef.id];
            if (!riskData) return null;
            const { score: livScore, label: livLabel } = calcRiskRating(riskData.probability, riskData.impact);
            return (
              <Card key={riskDef.id} className={cn(
                livScore >= 3 && riskData.applicable ? "border-red-200 bg-red-50/30" :
                livScore >= 2 && riskData.applicable ? "border-amber-200 bg-amber-50/20" : ""
              )}>
                <div className="flex items-start gap-3">
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      riskData.applicable ? "bg-brand border-brand text-white" : "bg-white border-gray-300"
                    )}
                  >
                    {riskData.applicable && <svg viewBox="0 0 12 12" fill="none" className="h-3 w-3"><path d="M2 6l3 3 5-5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      <span className="text-[10px] bg-red-100 text-red-700 rounded-full px-2 py-0.5 font-medium">{riskDef.category}</span>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {IMPACT_VALUES.map((v) => (
                              <button
                                key={v}
                                onClick={() => updateRiskItem(id, riskDef.id, riskData.probability, v)}
                                className={cn(
                                  "rounded-lg px-2.5 py-1 text-xs font-medium border transition-all",
                                  riskData.impact === v
                                    ? "bg-brand text-white border-brand"
      ...
      {activeTab === "opportunities" && (
        <div className="space-y-3 animate-fade-in">
          <div className="rounded-xl bg-green-50 border border-green-100 p-3 text-xs text-green-800 flex items-start gap-2 mb-4">
            ...
            Select Likelihood (how likely to materialise?) and Impact (how big is the upside?). Rating is derived automatically.
          </div>

          {OPPORTUNITY_DEFINITIONS.map((oppDef) => {
            const oppData = stage2.opportunities[oppDef.id];
            if (!oppData) return null;
            const { score: livScore, label: livLabel } = calcOpportunityRating(oppData.likelihood, oppData.impact);
            return (
              <Card key={oppDef.id} className={cn(
                ...
                <div className="flex items-start gap-3">
                  <div
                    ...
                    className={cn(
                      "mt-0.5 h-5 w-5 rounded flex items-center justify-center flex-shrink-0 cursor-pointer border-2 transition-colors",
                      oppData.applicable ? "bg-brand border-brand text-white" : "bg-white border-gray-300"
                    )}
                  >
                    {oppData.applicable && <svg viewBox="0 0 12 12" fill="none" className="h-3 w-3"><path d="M2 6l3 3 5-5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      <span className="text-[10px] bg-green-100 text-green-700 rounded-full px-2 py-0.5 font-medium">{oppDef.category}</span>
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {PROB_VALUES.map((v) => (
                              <button key={v}
                                onClick={() => updateOpportunityItem(id, oppDef.id, v, oppData.impact)}
                                className={cn(
                                  "rounded-lg px-2.5 py-1 text-xs font-medium border transition-all",
                                  oppData.likelihood === v ? "bg-brand text-white border-brand" : "bg-white text-gray-600 border-surface-border hover:border-brand"
                                )}
                              >{PROB_LABELS[v]}</button>
                            ))}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {IMPACT_VALUES.map((v) => (
                              <button key={v}
                                onClick={() => updateOpportunityItem(id, oppDef.id, oppData.likelihood, v)}
                                className={cn(
                                  "rounded-lg px-2.5 py-1 text-xs font-medium border transition-all",
                                  oppData.impact === v ? "bg-brand text-white border-brand" : "bg-white text-gray-600 border-surface-border hover:border-brand"
                                )}
                              >{IMPACT_LABELS[v]}</button>
                            ))}
                          </div>
                        </div>
                      </div>
      ...
      <div className="flex justify-between items-center mt-8 pt-6 border-t border-surface-border">
        ...
        <Button variant="primary" size="lg" rightIcon={<ArrowRight size={16} />} onClick={handleSaveAndNext}>
