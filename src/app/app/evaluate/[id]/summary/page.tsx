...
import {
  ArrowRight, MapPin, Tag, Layers, Rocket, Zap,
  Globe2, CheckCircle2, Info
} from "lucide-react";
...
export default function SummaryPage({ params }: Props) {
  const { id } = use(params);
  const { getEvaluation, computeSDGAlignment, updateSDGAlignment, updateStatus } = useEvaluationStore();
    ...
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500">Evaluation summary — review your context, then proceed to Stage I.</p>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-6">
        ...
        <Card>
          ...
          <dl className="space-y-2.5">
            {[
              ...
              { label: "NACE Division", value: `${ctx.naceCode} — ${ctx.naceLabel}`, icon: <Tag size={13} /> },
              { label: "Offering", value: ctx.offeringType, icon: <Layers size={13} /> },
              { label: "Status", value: ctx.launched ? "Launched" : "Pre-launch", icon: <Rocket size={13} /> },
              ...
              <div key={item.label} className="flex items-start gap-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  <span className="text-xs font-semibold text-gray-500 min-w-[90px]">{item.label}</span>
        ...
        <Card>
          ...
          <div className="space-y-2.5">
            {[
              { step: "Stage I", desc: "Financial + Environmental + Social/Governance", done: !!evaluation.stage1 },
              { step: "Stage II", desc: "Risks & Opportunities", done: !!evaluation.stage2 },
              { step: "Impact Summary", desc: "Material topics & priorities", done: !!evaluation.stage2 },
              { step: "SDG Alignment", desc: "Relevant SDGs for your model", done: !!evaluation.sdgAlignment },
              { step: "Dashboard", desc: "Full results & recommendations", done: evaluation.status === "completed" },
            ].map((item) => (
              <div key={item.step} className="flex items-center gap-3">
                <div className={`h-5 w-5 rounded-full flex items-center justify-center flex-shrink-0 ${item.done ? "bg-brand" : "bg-surface-border"}`}>
      ...
      <Card className="mb-6">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Based on your stage ({ctx.stage}) and business category (NACE {ctx.naceCode}), these SDGs are likely relevant.
            </p>
          </div>
        ...
        <div className="rounded-xl bg-amber-50 border border-amber-100 p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-amber-800">
            This is an early screening view, not the final SDG assessment.
            Your full SDG Alignment (combining Stage + NACE SDGs) will be generated after Stage II.
          </p>
        </div>
      </Card>
