    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/app/evaluations" className="text-brand font-semibold">
            ← Back to evaluations
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="no-print sticky top-0 z-50 bg-white border-b border-surface-border px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            href={`/app/evaluate/${id}/dashboard`}
      ...
      <div className="max-w-4xl mx-auto px-8 py-10">
        ...
        <div className="mb-10 pb-8 border-b-4 border-zeeus-green2">
          ...
          <div className="grid grid-cols-3 gap-4 mt-6">
            {summary
              ? [
                  ...
                  <div
                    key={stat.label}
                    className="rounded-xl border border-surface-border p-3 text-center"
                  >
          ...
          <div className="mt-6 rounded-xl bg-brand/5 border border-brand/15 p-4 text-xs text-gray-600 leading-relaxed">
            <strong className="text-brand-dark">Guidance tool, not a judgment tool.</strong> This
            report is generated by the ZEEUS Sustainability by Design (SbyD) Tool. Scores reflect
            qualitative assessments based on founder inputs aligned with ESRS dual materiality
            principles and UN SDG relevance. They are intended to guide design decisions, not to
            replace formal ESG reporting or legal compliance.
          </div>
        </div>
          ...
          <ReportSection title="2. Stage I — Financial Indicators" icon={<DollarSign size={18} />}>
            <div className="mb-3 flex items-center gap-3">
              ...
              <span className="text-xl font-black text-brand-dark">
                {evaluation.stage1.financial.roiIrrNpv +
                  evaluation.stage1.financial.sensitivityAnalysis +
                  evaluation.stage1.financial.uspStrategicFit +
                  evaluation.stage1.financial.marketGrowth}{' '}
                / 12
              </span>
            </div>
            <table className="w-full text-sm">
              ...
              <tbody>
                {FINANCIAL_KPIS.map((kpi) => {
                  const level =
                    evaluation.stage1!.financial[
                      kpi.id as keyof typeof evaluation.stage1.financial
                    ];
                  return (
        ...
        {evaluation.stage1 && (
          <ReportSection
            title="3. Stage I — Environmental Indicators (E1–E5)"
            icon={<Leaf size={18} />}
          >
            <table className="w-full text-sm">
              <thead>
                ...
                {envTopics.map((topic) => {
                  ...
                  return (
                    <tr key={topic.id} className="border-b border-surface-border/50">
                      ...
                      <td className="py-2 pr-4 font-bold text-brand-dark">
                        {a.applicable ? score.toFixed(2) : 'N/A'}
                      </td>
                      <td className="py-2 pr-4">
        ...
        {evaluation.stage1 && (
          <ReportSection
            title="4. Stage I — Social & Governance (S1–S4, G1)"
            icon={<Users size={18} />}
          >
            <table className="w-full text-sm">
              <thead>
            ...
            {summary && (
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-lg font-black text-gray-900">
                  {summary.riskOverallScore.toFixed(1)}/4
                </span>
                  ...
                  return (
                    <tr key={def.id} className="border-b border-surface-border/50">
                      ...
                      <td className="py-2 pr-4 text-xs text-gray-600">
                        {['N/A', 'Rare', 'Could occur', 'Likely', 'Very likely'][r.probability]}
                      </td>
                      <td className="py-2 pr-4 text-xs text-gray-600">
                        {['N/A', 'Low', 'Moderate', 'Significant', 'High'][r.impact]}
                      </td>
                      <td className="py-2">
                        <RiskChip label={r.ratingLabel} score={r.ratingScore} />
                      </td>
                    </tr>
                  );
                })}
          ...
          <ReportSection title="6. Stage II — Opportunities" icon={<TrendingUp size={18} />}>
            {summary && (
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-lg font-black text-gray-900">
                  {summary.opportunityOverallScore.toFixed(1)}/4
                </span>
                ...
                {OPPORTUNITY_DEFINITIONS.map((def) => {
                  ...
                  return (
                    <tr key={def.id} className="border-b border-surface-border/50">
                      ...
                      <td className="py-2 pr-4 text-xs text-gray-600">
                        {['N/A', 'Rare', 'Could occur', 'Likely', 'Very likely'][o.likelihood]}
                      </td>
                      <td className="py-2 pr-4 text-xs text-gray-600">
                        {['N/A', 'Low', 'Moderate', 'Significant', 'High'][o.impact]}
                      </td>
                      <td className="py-2">
                        <OpportunityChip label={o.ratingLabel} score={o.ratingScore} />
                      </td>
                    </tr>
                  );
                })}
              ...
              <>
                ...
                <ul className="space-y-1.5">
                  {summary.sensitivityHints.map((hint, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 rounded-lg p-2"
                    >
        ...
        <div className="border-t border-surface-border pt-6 text-center text-xs text-gray-400">
          ...
          <p>
            Developed by TUAS – Trier University of Applied Sciences, IfaS. Funded by the European
            Union (EIT).
          </p>
          <p className="mt-2">
            This is a guidance tool. Scores are based on self-reported inputs and do not constitute
