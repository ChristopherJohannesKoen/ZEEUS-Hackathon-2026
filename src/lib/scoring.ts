...
export function computeTopicScores(
  assessments: Record<string, ESGTopicAssessment>
): ComputedTopicScore[] {
...
export function calcRiskRating(
  probability: number,
  impact: number
): { score: number; label: RiskRating } {
  const score = RISK_MATRIX[probability]?.[impact] ?? 0;
  return { score, label: RISK_RATING_LABELS[score] as RiskRating };
}

export function calcOpportunityRating(
  likelihood: number,
  impact: number
): { score: number; label: OpportunityRating } {
...
export function calcOverallOpportunity(stage2: Stage2Data): {
  score: number;
  label: OpportunityRating;
} {
  const applicable = Object.values(stage2.opportunities).filter(
    (o) => o.applicable && o.ratingScore > 0
  );
  if (applicable.length === 0) return { score: 0, label: 'Neutral' };
...
export function calcConfidenceBand(stage1: Stage1Data): 'High' | 'Moderate' | 'Low' {
  const all = [...Object.values(stage1.environmental), ...Object.values(stage1.social)].filter(
    (t) => t.applicable
  );
...
export function calcSensitivityHints(stage1: Stage1Data): string[] {
  const hints: string[] = [];
  const allTopics = [...Object.values(stage1.environmental), ...Object.values(stage1.social)];
  ...
  allTopics.forEach((t) => {
    ...
    if (score >= 1.7 && score < 2) {
      hints.push(
        `${title} is close to becoming a relevant topic — consider reviewing your inputs.`
      );
    } else if (score >= 2.2 && score < 2.5) {
      hints.push(
        `${title} is approaching high-priority status — one step up in likelihood or magnitude would change your result.`
      );
    }
    ...
    if (t.applicable && t.evidenceBasis === 'assumed' && score >= 2) {
      hints.push(
        `${title} score is based on assumed inputs — measurement would increase confidence.`
      );
    }
  });
...
export function generateRecommendations(summary: Partial<DashboardSummary>): string[] {
  ...
  if ((summary.highPriorityTopics?.length ?? 0) > 0) {
    ...
    recs.push(
      `Prioritize ${names.join(', ')} — these topics have the highest potential for strategic impact. Build measurable action plans.`
    );
  }

  if ((summary.financialNormalized ?? 0) < 50) {
    recs.push(
      'Strengthen your financial case: run a sensitivity analysis, refine your USP, and gather market evidence to increase investor confidence.'
    );
  }

  if (summary.riskOverallLabel === 'Critical' || summary.riskOverallLabel === 'Severe') {
    recs.push(
      'Address high-severity risks now. Consider scenario planning, risk mitigation roadmaps, and early stakeholder engagement.'
    );
  }

  if (
    summary.opportunityOverallLabel === 'Great' ||
    summary.opportunityOverallLabel === 'Sustainable'
  ) {
    recs.push(
      'Your opportunity landscape looks strong — prioritize the top-rated opportunities and align them with your growth roadmap and pitch deck.'
    );
  }

  if (summary.confidenceBand === 'Low') {
    recs.push(
      'Many inputs are based on assumptions. As you prototype, replace assumed scores with estimated or measured data to strengthen your case.'
    );
  }

  if ((summary.materialTopics?.length ?? 0) > 0) {
    recs.push(
      'Share your material topics with your design team — each one is an opportunity to improve product-market fit and reduce future compliance burden.'
    );
  }

  if (recs.length === 0) {
    recs.push(
      'Your evaluation looks balanced. Keep monitoring material topics, update your SDG alignment as you scale, and revisit this assessment after your next milestone.'
    );
  }
...
export function computeDashboardSummary(evaluation: Evaluation): DashboardSummary | null {
  ...
  const { score: opportunityOverallScore, label: opportunityOverallLabel } =
    calcOverallOpportunity(stage2);
  ...
  const topOpportunities = Object.values(stage2.opportunities)
    ...
    .map((o) => {
      ...
      return {
        id: o.itemId,
        label: def?.title ?? o.itemId,
        score: o.ratingScore,
        rating: o.ratingLabel
      };
    });
...
export function generateCSV(evaluation: Evaluation): string {
  ...
  if (evaluation.stage2) {
    ...
    Object.values(evaluation.stage2.opportunities).forEach((o) => {
      rows.push(
        `Stage II – Opportunities,${o.itemId} Rating,${o.applicable ? o.ratingLabel : 'N/A'}`
      );
    });
  }
